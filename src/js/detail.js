let tem = window.localStorage.getItem('message')
let message = JSON.parse(tem)[0]
console.log(message)


function bandHtml() {
        $('.name').find('span').html(`${message.mingzi}`)
        $('.jd-price').html('京东价￥' + `${message.jiage}`)
        $('.yuantu').attr('src' , `${message.tupian}`)
        $('.datu').attr('src' , `${message.tupian}`)
        $('.xiaotu').attr('src' , `${message.tupian}`)
        console.log($('.yuantu'))
}
bandHtml()
